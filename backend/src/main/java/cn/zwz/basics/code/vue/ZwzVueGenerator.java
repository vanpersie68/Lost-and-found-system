...
@ApiOperation(value = "Frontend Code Generator")  // 前端代码生成器
@RestController
@Api(tags = "Vue Code Generation")  // Vue代码生成
...
public class ZwzVueGenerator {
    ...
    @ApiOperation(value = "Generate Frontend Vue Table Code")  // 生成前端Vue表格代码
    ...
    @ApiOperation(value = "Generate Frontend Vue Tree Structure Code")  // 生成前端Vue树形代码
    ...
    @ApiOperation(value = "Generate Frontend Code for Entity Class")  // 生成实体类的前端代码
    public Result<Object> getEntityData(@PathVariable String path) {
        String result = null;
        try {
            result = generateEntityData(path);
        } catch (Exception e) {
            return ResultUtil.error("The entity class has been deleted");  // 实体类已被删除
        }
    ...
    public String generate(String template, boolean api, String vueName, Integer rowNum, List<Field> fields) throws IOException {
        ...
        if(rowNum == 1){
            ...
            // 不满足的情况
            throw new ZwzException("The input row can only be between 1 to 4 columns");  // 一行的输入栏只能为1到4列
        }
    ...
    public static String name(String name, boolean isFirstUpper){
        ...
        if(StrUtil.isBlank(name)){
            throw new ZwzException("name cannot be empty");  // name不能为空
        }
