...
@ApiOperation(value = "Pagination Utility Class") //分页工具类
public class PageUtil {
    ...
    @ApiModelProperty(value = "JPA pagination method") //JPA分页方法
    ...
    @ApiModelProperty(value = "MybatisPlus pagination method") //MybatisPlus分页方法
    ...
    @ApiModelProperty(value = "custom pagination method") //自定义分页方法
    ...
    @ApiModelProperty(value = "Preventing SQL Injection Attacks in MyBatis Plus") //防 Mybatis Plus 的 SQL 注入攻击
    public static void SQLInject(String sqlStr){
        ...
        for(int i = 0; i < NO_CAN_USE_WORDS.length; i ++) {
            if (sqlStr.contains(NO_CAN_USE_WORDS[i])) {
                throw new ZwzException(sqlStr + "Invalid word"); //单词不合法
            }
        }
    }

    @ApiModelProperty(value = "convert camelCase to snake_case") //驼峰转下划线
