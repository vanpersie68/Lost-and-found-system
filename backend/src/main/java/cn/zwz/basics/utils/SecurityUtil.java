...
@ApiOperation(value = "Authentication tools") //鉴权工具类
@Component
public class SecurityUtil {
    ...
    @ApiOperation(value = "Get new user Token") //获取新的用户Token
    public String getToken(String username, Boolean saveLogin){
        if(ZwzNullUtils.isNull(username)){
            throw new ZwzException("username cannot be empty"); //username不能为空
        }
        boolean saved = false;
    ...
    @ApiOperation(value = "Query the permission list of a specified user") //查询指定用户的权限列表
    ...
    @ApiOperation(value = "Query the user object of the current Token") //查询当前Token的用户对象
    public User getCurrUser(){
        ...
        if(Objects.equals("anonymousUser",selectUser.toString())){
            throw new ZwzException("Login invalid"); //登录失效
        }
