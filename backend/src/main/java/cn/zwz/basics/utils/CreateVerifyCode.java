...
@ApiOperation(value = "Verification Code Utility") //验证码工具类
public class CreateVerifyCode {

    @ApiModelProperty(value = "Verification Code") //验证码
    private String code = null;

    @ApiModelProperty(value = "Number of Characters in the Verification Code") //验证码字符个数
    private int charactersNumber = 4;

    @ApiModelProperty(value = "Image Height") //图片高度
    private int imagePeripheralHeight = 40;

    @ApiModelProperty(value = "Image Width") //图片宽度
    private int imagePeripheralWidth = 160;

    @ApiModelProperty(value = "Number of Interference Lines") //干扰线数
    private int lineCount = 20;

    @ApiModelProperty(value = "Image Buffer") //图片Buffer
    ...
    @ApiOperation(value = "Image Generation Utility Class") //图片生成工具类
    private void creatImage(String code) {
        if(ZwzNullUtils.isNull(code)){
            throw new ZwzException("The image verification code has expired. Let's generate a new one!"); //图形验证码过期了，再生成个新的哦!
        }
        this.code = code;
        buffImg = new BufferedImage(imagePeripheralWidth, imagePeripheralHeight, BufferedImage.TYPE_INT_RGB);
        Graphics g = buffImg.getGraphics();
    ...
    @ApiOperation(value = "To randomly generate a verification code") //随机生成验证码
    ...
    @ApiOperation(value = "To randomly generate verification code colors") //随机生成验证码颜色
