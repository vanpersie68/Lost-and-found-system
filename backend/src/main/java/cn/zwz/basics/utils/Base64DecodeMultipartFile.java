...
@ApiOperation(value = "File Utility Class") //文件工具类
public class Base64DecodeMultipartFile implements MultipartFile {

    @ApiModelProperty(value = "image content") //图片内容
    private final byte[] imgContent;

    @ApiModelProperty(value = "image caption") //图片抬头
    ...
    @Override
    @ApiOperation(value = "get the size of an image") //获取图片的大小
    public long getSize() {
        return imgContent.length;
    }

    @Override
    @ApiOperation(value = "get the input stream of an image") //获取图片的输入流
    public InputStream getInputStream() {
        return new ByteArrayInputStream(imgContent);
    }

    @Override
    @ApiOperation(value = "get the byte stream of an image") //获取图片的字节流
    public byte[] getBytes() {
        return imgContent;
    }

    @Override
    @ApiOperation(value = "get the image name") //获取图片名称
    public String getName() {
        return System.currentTimeMillis() + Math.random() + IMAGE_PATH_STEP_STR + header.split(IMAGE_HEADER_SPLIT_STR)[1];
    }

    @Override
    @ApiOperation(value = "image conversion") //图片转换
    public void transferTo(File dest) throws IOException, IllegalStateException {
        new FileOutputStream(dest).write(imgContent);
    }

    @Override
    @ApiOperation(value = "check if the image is empty") //判断图片是否为空
    public boolean isEmpty() {
        return imgContent == null || imgContent.length == 0;
    }

    @Override
    @ApiOperation(value = "get the complete image path") //获取图片完整路径
    public String getOriginalFilename() {
        return System.currentTimeMillis() + (int)Math.random() * 10000 + IMAGE_PATH_STEP_STR + header.split(IMAGE_HEADER_SPLIT_STR)[1];
    }

    @Override
    @ApiOperation(value = "get the image type") //获取图片类型
