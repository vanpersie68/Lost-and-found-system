...
@ApiOperation(value = "Redis Exception Handling") //Redis异常处理
@Configuration
@Slf4j
public class RedisExceptionThrowsConfig extends CachingConfigurerSupport {

    @ApiModelProperty(value = "Duration Type") //时长类型
    private String unit = "day";

    @ApiModelProperty(value = "Duration Value", notes = "-1 means no limit") //时长值 -1为不限制
    private Integer time = 30;

    @Override
    @ApiOperation(value = "Redis Serialization Exception") //Redis序列化异常
    public CacheErrorHandler errorHandler() {
        CacheErrorHandler ceh = new CacheErrorHandler() {
            @Override
            public void handleCacheGetError(RuntimeException e, Cache cache, Object key) {
                log.warn("An error occurred during Redis query serialization");  //Redis序列化出现了查询异常
                log.warn(key.toString());
            }
            @Override
            public void handleCachePutError(RuntimeException e, Cache cache, Object key, Object value) {
                log.warn("An error occurred during Redis insert serialization"); //Redis序列化出现了插入异常
                log.warn(key.toString());
            }
            @Override
            public void handleCacheEvictError(RuntimeException e, Cache cache, Object key) {
                log.warn("An error occurred during Redis Evict serialization"); //Redis序列化出现了Evict异常
                log.warn(key.toString());
            }
            @Override
            public void handleCacheClearError(RuntimeException e, Cache cache) {
                log.warn("An error occurred during Redis delete serialization"); //Redis序列化出现了删除异常
            }
        };
        return ceh;
    }
    ...
    @Bean
    @ApiOperation(value = "Redis Serialization")  //Redis序列化
