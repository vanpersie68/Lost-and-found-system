...
@ApiOperation(value = "Log Implementation Class")   // 日志实现类
@Aspect
@Component
@Slf4j
public class SystemLogAspect {
    ...
    @ApiOperation(value = "Controller Layer Aspect")  // 控制层切点
    @Pointcut("@annotation(cn.zwz.basics.log.SystemLog)")
    public void controllerAspect() {

    }

    @ApiOperation(value = "Before Advice") // 前置通知
    ...
    @ApiOperation(value = "After Returning Advice") //后置通知
    ...
    public void after(JoinPoint joinPoint) {
        try {
            ...
            String device = "", isMobile = "PC Side";
            UserAgent ua = UserAgentUtil.parse(request.getHeader("user-agent"));
            if (ua != null) {
                if (ua.isMobile()) {
                    isMobile = "Mobile Side";
                }
            }
