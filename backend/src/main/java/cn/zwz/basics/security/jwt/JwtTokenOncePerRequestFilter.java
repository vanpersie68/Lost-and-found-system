...
@ApiOperation(value = "Custom permission filtering") //自定义权限过滤
@Slf4j
public class JwtTokenOncePerRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            log.warn("Custom permission filtering failed" + e); //自定义权限过滤失败
        }
        filterChain.doFilter(request, response);
    }

    @ApiOperation(value = "Determine if the login has expired") //判断登录是否失效
    private UsernamePasswordAuthenticationToken getUsernamePasswordAuthenticationToken(String header, HttpServletResponse response) {
        ...
        if(ZwzNullUtils.isNull(tokenInRedis)){
            ResponseUtil.out(response, ResponseUtil.resultMap(RESPONSE_FAIL_FLAG,RESPONSE_NO_ROLE_CODE,"Login status has expired, need to log in again!")); //登录状态失效，需要重登！
            return null;
        }
