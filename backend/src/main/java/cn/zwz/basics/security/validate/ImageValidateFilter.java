...
@ApiOperation(value = "Verification Code Filter Class") //验证码过滤类
@Configuration
public class ImageValidateFilter extends OncePerRequestFilter {
    ...
    @Override
    @ApiOperation(value = "Verification Code Filtering") //验证码过滤
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(ZwzNullUtils.isNull(userInputCode) || ZwzNullUtils.isNull(verificationCodeId)){
            ResponseUtil.out(response, ResponseUtil.resultMap(RESPONSE_FAIL_FLAG,RESPONSE_CODE_FAIL_CODE,"Verification code is empty")); //验证码为空
            return;
        }
        String codeAnsInRedis = redisTemplate.opsForValue().get(verificationCodeId);
        if(ZwzNullUtils.isNull(codeAnsInRedis)){
            ResponseUtil.out(response, ResponseUtil.resultMap(RESPONSE_FAIL_FLAG,RESPONSE_CODE_FAIL_CODE,"The verification code has expired and needs to be re-entered")); // 已过期的验证码，需要重新填写
            return;
        }
        if(!Objects.equals(codeAnsInRedis.toLowerCase(),userInputCode.toLowerCase())) {
            ResponseUtil.out(response, ResponseUtil.resultMap(RESPONSE_FAIL_FLAG,RESPONSE_CODE_FAIL_CODE,"The verification code is incorrect")); //验证码不正确
            return;
        }
