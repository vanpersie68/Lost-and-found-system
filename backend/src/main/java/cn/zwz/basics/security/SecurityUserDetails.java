...
@ApiOperation(value = "Query user's roles and menu permissions") //查询用户的角色和菜单权限
public class SecurityUserDetails extends User implements UserDetails {
    ...
    @Override
    @ApiOperation(value = "Query the user's roles and menu permissions") //查询用户的角色和菜单权限
    ...
    @Override
    @ApiOperation(value = "Is account enabled?") //账号是否启用
    public boolean isEnabled() {
        return Objects.equals(CommonConstant.USER_STATUS_NORMAL,this.getStatus());
    }

    @ApiOperation(value = "Is account not expired?") //账号是否过期
    @Override
    ...
    @Override
    @ApiOperation(value = "Is account password not expired?") //账号密码是否过期
    public boolean isCredentialsNonExpired() {
        return true;
    }

    @Override
    @ApiOperation(value = "Is account not locked?") //账号是否禁用
