...
@ApiOperation(value = "Login decision class") //登录判断类
@Component
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    @Override
    @ApiOperation(value = "Retrieve all user information based on the account/phone number") //根据账号/手机号查询用户所有信息
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
        ...
        if(!ZwzNullUtils.isNull(value)){
            throw new UsernameNotFoundException("Trial limit exceeded. Please try again in " + timeRest + " minutes.");
            //试错超限，请您在 分钟后再登
        }
