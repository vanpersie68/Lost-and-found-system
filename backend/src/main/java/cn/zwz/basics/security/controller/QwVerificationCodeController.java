...
@RestController
@Api(tags = "Enterprise WeChat Verification Code Login Interface") //企业微信验证码登录接口
...
public class QwVerificationCodeController {
    ...
    @ApiOperation(value = "Send Enterprise WeChat Verification Code")
    //发送企微验证码
    ...
    public Result<Object> sendVerificationCode(@RequestParam String jobNumber) {
        if(!Objects.equals("zwz",jobNumber)) {
            return ResultUtil.error("Please contact the administrator to configure your employee number");
            //请联系管理员配置您的工号
        }
        ...
        if(userCount < 1L) {
            return new ResultUtil<Object>().setErrorMsg("No permission to log in");
            //无权限登入
        }
         ...
         * 这里需要实现判断发给谁的业务逻辑
         */
        ZwzWxNoticeUtils.sendTuWenMessage("zwz", "OA login verification", "Verification code " + verificationCode + " will expire in 1 minute", "https://gitee.com/yyzwz", "https://bkimg.cdn.bcebos.com/pic/37d12f2eb9389b503a80d4b38b35e5dde6116ed7", WeiChatUtils.getToken());
        redisTemplate.set("qwsms:" + jobNumber, verificationCode, 60, TimeUnit.SECONDS);
        return ResultUtil.success();
    }

    @SystemLog(about = "Enterprise WeChat verification code login", type = LogType.LOGIN)
    @ApiOperation(value = "Enterprise WeChat verification code login")
    ...
    public Result<Object> verificationCodeLogin(@RequestParam String jobNumber, @RequestParam String code) {
        ...
        if(codeAns == null) {
            return ResultUtil.error("Verification code has expired");
        }
        if(codeAns.equals(code)) {
            ...
            if(users.size() == 0) {
                return ResultUtil.error(jobNumber + " account does not exist");
            }
        ...
        return ResultUtil.error("Verification code is incorrect");
    }
}
