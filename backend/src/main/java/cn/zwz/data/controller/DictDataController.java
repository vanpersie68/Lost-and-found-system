...
@Api(tags = "Dictionary data value interface") //字典数据值接口
@Transactional
public class DictDataController {
    ...
    @SystemLog(about = "Query the value of a single data dictionary", type = LogType.DATA_CENTER,doType = "DICT_DATA-01")
    @RequestMapping(value = "/getByType/{type}", method = RequestMethod.GET)
    @ApiOperation(value = "Query the value of a single data dictionary")
    public Result<Object> getByType(@PathVariable String type){
        QueryWrapper<Dict> qw = new QueryWrapper<>();
        qw.eq("type",type);
        Dict selectDict = iDictService.getOne(qw);
        if (selectDict == null) {
            return ResultUtil.error("dictionary "+ type +" does not exist");
        }
    ...
    @SystemLog(about = "Query data dictionary value", type = LogType.DATA_CENTER,doType = "DICT_DATA-02")
    @RequestMapping(value = "/getByCondition", method = RequestMethod.GET)
    @ApiOperation(value = "Query data dictionary value")
    ...
    @SystemLog(about = "Delete data dictionary value", type = LogType.DATA_CENTER,doType = "DICT_DATA-03")
    @RequestMapping(value = "/delByIds", method = RequestMethod.POST)
    @ApiOperation(value = "Delete data dictionary value")
    ...
    @SystemLog(about = "Add data dictionary value", type = LogType.DATA_CENTER,doType = "DICT_DATA-04")
    @RequestMapping(value = "/add", method = RequestMethod.POST)
    @ApiOperation(value = "Add data dictionary value")
    public Result<Object> add(DictData dictData){
        Dict selectDict = iDictService.getById(dictData.getDictId());
        if (selectDict == null) {
            return ResultUtil.error("Dictionary does not exist");
        }
    ...
    @SystemLog(about = "Edit data dictionary values", type = LogType.DATA_CENTER,doType = "DICT_DATA-05")
    @RequestMapping(value = "/edit", method = RequestMethod.POST)
    @ApiOperation(value = "Edit data dictionary values")
