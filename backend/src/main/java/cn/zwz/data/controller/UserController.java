...
public class UserController {
    ...
    @SystemLog(about = "Get the currently logged in user", type = LogType.DATA_CENTER,doType = "USER-02")
    ...
    public Result<Object> regist(@Valid User u){
        ...
        if(iUserService.count(userQw) > 0L) {
            return ResultUtil.error("Duplicate login account/mobile phone number");
        }
    ...
    @SystemLog(about = "Unlock verification password", type = LogType.DATA_CENTER,doType = "USER-03")
    @RequestMapping(value = "/unlock", method = RequestMethod.POST)
    @ApiOperation(value = "Unlock verification password")
    public Result<Object> unLock(@RequestParam String password){
        User u = securityUtil.getCurrUser();
        if(!new BCryptPasswordEncoder().matches(password, u.getPassword())){
            return ResultUtil.error("Incorrect password");
        }
        return ResultUtil.data(null);
    }

    @SystemLog(about = "reset Password", type = LogType.DATA_CENTER,doType = "USER-04")
    ...
    @SystemLog(about = "Modify user information", type = LogType.DATA_CENTER,doType = "USER-05")
    @RequestMapping(value = "/edit", method = RequestMethod.POST)
    @ApiOperation(value = "Modify user information",notes = "The username and password will not be modified. Username needs to be updated to cache.")
    ...
    public Result<Object> editOwn(User u){
        ...
        iUserService.saveOrUpdate(u);
        return ResultUtil.success("Successfully modified");
    }

    @SystemLog(about = "change Password", type = LogType.DATA_CENTER,doType = "USER-06")
    ...
    public Result<Object> modifyPass(@RequestParam String password,@RequestParam String newPass,@RequestParam String passStrength){
        ...
        if(!new BCryptPasswordEncoder().matches(password, user.getPassword())){
            return ResultUtil.error("The original password is incorrect");
        }
    ...
    @SystemLog(about = "Query user", type = LogType.DATA_CENTER,doType = "USER-07")
    ...
    @SystemLog(about = "Query users by department", type = LogType.DATA_CENTER,doType = "USER-08")
    @RequestMapping(value = "/getByDepartmentId", method = RequestMethod.GET)
    @ApiOperation(value = "Query users by department")
    ...
    @SystemLog(about = "Simulate search user", type = LogType.DATA_CENTER,doType = "USER-09")
    @RequestMapping(value = "/searchByName/{username}", method = RequestMethod.GET)
    @ApiOperation(value = "Simulate search user")
    ...
    @SystemLog(about = "Query all users", type = LogType.DATA_CENTER,doType = "USER-10")
    ...
    @SystemLog(about = "Administrator modifies information", type = LogType.DATA_CENTER,doType = "USER-11")
    @RequestMapping(value = "/admin/edit", method = RequestMethod.POST)
    @ApiOperation(value = "Administrator modifies information")
    @CacheEvict(key = "#u.username")
    public Result<Object> edit(User u,@RequestParam(required = false) String[] roleIds){
        ...
        if(!Objects.equals(customaryUser.getMobile(),u.getMobile())) {
            ...
            if(customaryUserCount > 0) {
                return ResultUtil.error("Duplicate mobile phone number");
            }
        }
    ...
    @SystemLog(about = "Add user", type = LogType.DATA_CENTER,doType = "USER-12")
    ...
    @SystemLog(about = "Disable user", type = LogType.DATA_CENTER,doType = "USER-13")
    ...
    @SystemLog(about = "enable user", type = LogType.DATA_CENTER,doType = "USER-14")
    ...
    @SystemLog(about = "delete users", type = LogType.DATA_CENTER,doType = "USER-15")
    ...
    @SystemLog(about = "Import users", type = LogType.DATA_CENTER,doType = "USER-16")
    ...
    public Result<Object> importData(@RequestBody List<User> users){
        ...
        for(User u: users){
            ...
            if(StrUtil.isBlank(u.getUsername())||StrUtil.isBlank(u.getPassword())){
                errors.add(count);
                reasons.add("Account password is empty");
                continue;
            }
            ...
            if(iUserService.count(userQw) > 0L) {
                errors.add(count);
                reasons.add("Username already exists");
                continue;
            }
            u.setPassword(new BCryptPasswordEncoder().encode(u.getPassword()));
            if(StrUtil.isNotBlank(u.getDepartmentId())){
                ...
                if(department == null) {
                    errors.add(count);
                    reasons.add("Department does not exist");
                    continue;
                }
            }
            if(u.getStatus()==null){
        ...
        String successMessage = "Department does not exist " + successCount + " users";
        String failMessage = "Successfully imported " + successCount + " users, with " + errors.size() + " users failed.<br>"
                + "Data import failed for rows " + errors.toString() + ", and the reason is:<br>"
                + reasons.toString();
        String message = null;
    ...
    @ApiOperation(value = "Add user's role and menu information")
